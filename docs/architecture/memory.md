...
| Recall (UserPromptSubmit hook) | **ContextAssembler** (`assemble.rs`), five parallel moves merged by confidence with a coverage floor and per-move quotas: M1 chunk content match (FTS5; the vector arm fires only when the caller passes a pre-computed query embedding — the hot path never constructs an embedder), M5 fact relevance (dual-weighted BM25 over the subject+predicate+object `facts_fts` + slug arm + optional KNN, RRF-fused; importance breaks ties only — added 2026-08-18 after the golden-set audit measured 3/17 recall), M2 entity-subject facts, M3 predicate-cue facts, M4 temporal facts (direct SQL over `facts`, importance/recency ordered — not FTS). Trivial/short prompts deliberately get **zero injection**; output hard-capped at 3k chars with the chunk render cap enforced in the merge budget; per-move stats logged to `.hex/memory/recall-log.jsonl`. Regression gate: `hex memory eval` (golden set + no-regression baseline). |
