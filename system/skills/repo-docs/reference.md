...
- **Maintenance record** (in-repo, an ADD op, behind a pointer — e.g. `docs/doc-maintenance.md`): the source-dir → doc mapping table (the routing backbone — name an owner); each claim's reproducible check command, machine-readable; declined changes and do-NOT-touch rationale; UNVERIFIED claims. Optional provenance anchors per claim: content hash or commit SHA of the verified source (evidence provenance — not edit-date stamps, which stay banned). Future runs read it in Phase 0; prior declined-with-reason items stand as refutations unless the cited code changed. Propose a recurring CI gate (link checker + drift/freshness check) in Upstream proposals — detection that runs on every merge beats a one-shot pass.
