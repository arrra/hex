...
/// Pure mapping: event name + data → the (scope, key, value) state write.
///
...
/// the key so distinct events fire distinct triggers. The supplied data is
/// the state value.
///
/// No engine, no network — unit-testable.
pub fn emit_target(event: &str, data: &Value) -> EmitTarget {
    EmitTarget {
...
pub fn emit(event: &str, data: Value) -> Result<(), String> {
    let target = emit_target(event, &data);
...
mod tests {
    ...
    fn emit_target_uses_events_scope_and_event_name_as_key() {
        let data = json!({"a": 1});
        let t = emit_target("foo.bar", &data);
    ...
    fn emit_target_is_pure() {
        let data = json!({"k": "v"});
        let a = emit_target("e", &data);
        let b = emit_target("e", &data);
        assert_eq!(a, b);
    }
}
