#!/usr/bin/env bash
# Fetch a PR's diff and post a code review via OpenAI gpt-5 (or $CODEX_PR_MODEL).
# Called by hex-events on github.pr.opened. Idempotent: skips if review comment exists.
set -uo pipefail
...
MODEL="${CODEX_PR_MODEL:-gpt-5}"
OPENAI_API_KEY="${OPENAI_API_KEY:-}"
COMMENT_PREFIX="🤖 Auto-review (hex-events + Codex / gpt-5)"
MAX_DIFF_LINES=5000
LOG_DIR="$HOME/.hex-events/logs"
LOG_FILE="$LOG_DIR/codex-pr-review.log"
...
# Step 2: idempotency — skip if review comment already posted
...
if [[ "$EXISTING_COMMENT" != "0" ]]; then
  _log "skip" "review comment already exists"
  exit 0
fi
...
[Diff truncated at $MAX_DIFF_LINES lines (full diff: $DIFF_LINES lines). Review covers first $MAX_DIFF_LINES lines only.]"
  MAX_TOKENS=4096
...
SYSTEM_PROMPT="You are a senior software engineer conducting a thorough code review. Be specific, actionable, and direct. Reference file paths and line numbers where relevant."
...
_log "done" "review posted successfully"
exit 0
