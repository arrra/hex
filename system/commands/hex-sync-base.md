...
4. **Apply approved changes**
   ...
   - Note: hex uses `.claude/` but hex uses `dot-claude/` (renamed during install)
...
Before syncing, detect the upstream repo layout:
- **v1 layout** (legacy `mrap/hex`): has `dot-claude/` at repo root
- **v2 layout** (`mrap/hex-foundation`): has `system/` and `templates/` at repo root

Path map by layout:

| Local path | v1 upstream path | v2 upstream path |
|------------|------------------|------------------|
| `.hex/scripts/` | `dot-claude/scripts/` | `system/scripts/` |
| `.hex/skills/` | `dot-claude/skills/` | `system/skills/` |
| `.claude/commands/` | `dot-claude/commands/` | `system/commands/` |
| `.hex/hooks/` | `dot-claude/hooks/` | `system/hooks/` (if present) |
| `CLAUDE.md` | `CLAUDE.md` | `templates/CLAUDE.md` |

Detect the upstream layout inline (the `system/` dir marks the v2 foundation layout;
`dot-claude/` marks the legacy v1 layout):
```bash
if [ -d "$BASE_DIR/system" ]; then LAYOUT=v2
elif [ -d "$BASE_DIR/dot-claude" ]; then LAYOUT=v1
else LAYOUT=unknown; fi
```
Then translate paths using the mapping table above.
