...
# Cron: */30 * * * * cd "${HEX_DIR:-$HOME/hex}" && bash .hex/scripts/meeting-prep.sh
set -uo pipefail
...
# --- LLM CLI check ---
if [ -z "$LLM_CLI" ]; then
    log "No LLM CLI found (need claude or codex), skip."
    exit 0
fi
...
log "Calling $LLM_CLI (prompt: $(wc -c < "$PROMPT_TMP" | tr -d ' ') bytes)..."
