...
| 7 | **Execute safely — all work in worktrees.** ALL repository work — every edit to any git repo, interactive sessions and BOI workers alike — happens in a git worktree (minimum) or container (preferred), never the shared checkout. Concurrent agents in one working tree silently clobber each other's uncommitted edits and tangle the shared index. Never mutate production in place. Review integrations for exfiltration/injection before wiring up. Never connect untested code to credentials. (consolidates #8, #15) |
