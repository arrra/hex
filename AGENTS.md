...
- **User-added files in `commands/`, `skills/`, `scripts/`, `hooks/` survive the upgrade deletion pass if they contain `<!-- hex:user-local -->`.** Add this HTML comment anywhere in `~/hex/.hex/{commands,skills,scripts,hooks}/<your-file>` and `hex upgrade`'s deletion pass preserves it across syncs. The marker guards against *deletion* only — a file the foundation also ships is still overwritten by `apply_sync`; the marker does not fork foundation content. Without the marker, user-added files in these managed directories are deleted (treated as stale foundation files). Backup is always written to `~/hex/.hex/.upgrade-backup-YYYYMMDD-HHMMSS/`.
