...
- **User-customized files in `commands/`, `skills/`, `scripts/`, `hooks/` survive upgrades if they contain `<!-- hex:user-local -->`.** Add this HTML comment anywhere in `~/hex/.hex/{commands,skills,scripts,hooks}/<your-file>` and `hex upgrade`'s deletion pass preserves it across syncs. Without the marker, user-added files in these managed directories are deleted (treated as stale foundation files). Backup is always written to `~/hex/.hex/.upgrade-backup-YYYYMMDD-HHMMSS/`.
